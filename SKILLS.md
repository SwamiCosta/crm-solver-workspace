...
## SK-12 — Safe Deletion Protocol

**Applies to:** Purger  
**Trigger:** Before any irreversible deletion — data DELETE, file removal, or table DROP

This skill defines the mandatory gates and execution pattern for all destructive operations in Phase 5. Every gate is blocking — the operation does not proceed until it passes.

### Pre-deletion gates

All four gates must pass before any Phase 5 destructive action begins. Gates G-1 through G-3 are checked once at the start of Phase 5. Gate G-4 is checked per operation batch.

| Gate | Condition | Failure action |
|---|---|---|
| G-1 | Fixer completion report present in `docs/migration-reports/` | Stop — escalate to Overseer |
| G-2 | Diagnoser zero-anomaly scan on `_new` tables — human sign-off received | Stop — request operator to run Diagnoser |
| G-3 | Fresh DB backup generated, accessible, and verified (file size > 0) | Stop — do not proceed without confirmed backup |
| G-4 | Human has provided explicit written approval for this specific operation scope | Stop — present for approval per rule 4.1 |

### Data deletion execution pattern

For each DELETE batch:

```sql
-- Step 1: Bounded SELECT preview (per SK-08)
SELECT id, <key_fields>
FROM <entity>
WHERE migrated = TRUE
ORDER BY id
LIMIT 500 OFFSET <batch_offset>;

-- Step 2: Generate batch report → human approval (rule 4.1)

-- Step 3: Execute in transaction
BEGIN;
DELETE FROM <entity>
WHERE id IN (<ids_from_step_1>);
-- Verify affected row count matches preview count
COMMIT;  -- ROLLBACK if mismatch
```

### Table DROP — separate gate required

A DROP TABLE is a distinct action from record deletion and requires a separate approval:

```sql
-- Only after: all records deleted AND explicit written client sign-off received
DROP TABLE <entity> CASCADE;
```

The client's written authorisation must explicitly include the word **"drop"** and the table name. Approval to delete records does not substitute for this authorisation.

### File deletion — replacement must be named

Each file removed in a code purge PR must have a corresponding active V2 replacement identified in the same PR description:

- **Removed:** `routes/contacts.js` (legacy endpoint handler)
- **Replaced by:** `routes/v2/contacts.js` (active since PR #X, merged YYYY-MM-DD)

A file may not be removed unless a named, deployed replacement is confirmed.

### What is never deleted

The following artifacts are permanently exempt from all deletion operations:
- `audit_log` table and all its rows — preserved beyond Phase 5
- Any `_new` table or its rows — these are the active production tables
- The Interfacer container and its configuration
- Pre-Phase 5 backup files generated by the Fixer at V2 cutover
- The pre-purge backup generated at the start of Phase 5
